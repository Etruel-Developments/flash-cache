...
class flash_cache_posts {
	...
	*/
	public static function delete_cache_action() {
		...
		$post_id = $_REQUEST['id'];
		if (empty($post_id)) {
			flash_cache_notices::add(__('Invalid post id.', 'flash-cache' ));
			wp_redirect(base64_decode($_GET['referer']));
			exit;
		}
		...
		$cache_path = $cache_dir.$_SERVER['SERVER_NAME'].$post_path;
		...
		if ($cache_dir.$_SERVER['SERVER_NAME'] != $cache_path) {
	...
	*/
	public static function delete_cache_button() {
		...
		$delete_url = wp_nonce_url(admin_url('admin-post.php?id=' . absint( $post->ID ) . '&action=flash_cache_delete_cache&referer='. esc_attr(base64_encode(wp_unslash( $_SERVER['REQUEST_URI'] ))  )), 'delete_cache_nonce', '_nonce');
